...
public record Course(String title, String description, int credits, LocalDate startDate) {
    
    private static final Logger logger = Logger.getLogger(Student.class.getName());
    
     public Course(String title, String description, int credits, LocalDate startDate) {
        if (CourseUtils.isValidDescription(description) && CourseUtils.isValidCredit(credits) && CourseUtils.isValidstartDate(startDate) && CourseUtils.isValidTitle(title)) {
        this.title = title;
    ...
    static Course createCourse(String title, String description, int credits, LocalDate startDate) {
        if (CourseUtils.isValidTitle(title) && CourseUtils.isValidDescription(description) && CourseUtils.isValidCredit(credits) && CourseUtils.isValidstartDate(startDate) ) {
            logger.log(Level.INFO, "Course created successfully: {0}, {1}, {2}, {3}",
                new Object[]{title, description, credits, startDate});
    ...
    public String getFullName() {
        ...
        String fullName = title.substring(0, 3).toUpperCase() + "-" + description.substring(0, 3).toUpperCase() + "-" + credits + startDate.toString();   
        return fullName;
    }
